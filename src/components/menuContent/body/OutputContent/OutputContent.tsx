...
import TriggerSlackForm from '../TriggerPointContent/TriggerSlackForm'
...
export default function OutputContent() {
  ...
  const [selected, setSelected] = useState<OutputOption>(state.output.destination)
  ...
  return (
    <section>
      ...
      <div className="flex flex-col gap-4 items-center">
        <div className={selected === 'slack' ? 'ring-2 ring-violet-400/50 rounded-2xl' : ''}>
          <SlackTrigger
            isOpen={selected === 'slack'}
            onToggle={() => setSelected(prev => (prev === 'slack' ? null : 'slack'))}
            value={slackValue}
            onValueChange={setSlackValue}
          />
        </div>
        <div className={selected === 'telegram' ? 'ring-2 ring-cyan-400/50 rounded-2xl' : ''}>
          <TelegramTrigger
            isOpen={selected === 'telegram'}
            onToggle={() => setSelected(prev => (prev === 'telegram' ? null : 'telegram'))}
          />
          {selected === 'telegram' && (
            <div className="mt-3 w-[600px]">
              <TriggerTelegramForm value={telegramValue} onValueChange={setTelegramValue} />
            </div>
          )}
        </div>
      </div>
